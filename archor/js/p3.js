(function () {
  ...
  var anchorPickerModal = document.getElementById("anchor-picker-modal");
  var anchorPicksRoot = document.getElementById("anchor-picks");
  var anchorPickerFooter = document.getElementById("anchor-picker-footer");
  var anchorPickerCount = document.getElementById("anchor-picker-count");
  var anchorPickerConfirm = document.getElementById("anchor-picker-confirm");
  var transportModal = document.getElementById("transport-modal");
  var transportModalSub = document.getElementById("transport-modal-sub");
  var transportLegsPreview = document.getElementById("transport-legs-preview");
  var transportEdit = document.getElementById("transport-edit");
  var transportEditList = document.getElementById("transport-edit-list");
  var transportActions = document.getElementById("transport-actions");
  var transportAcceptBtn = document.getElementById("transport-accept");
  var transportModifyBtn = document.getElementById("transport-modify");
  var transportEditConfirmBtn = document.getElementById("transport-edit-confirm");
  ...
  var PROGRESS_STEPS = [
    { id: "frame", label: "路书框架" },
    { id: "route", label: "动线与 POI" },
    { id: "diag", label: "AI 行程诊断" },
    { id: "render", label: "渲染完成" },
  ];
  ...
  var PLAN_CONTENT_VERSION = "play-multi-v1";

  var TRANSPORT_MODES = ["汽车", "地铁", "骑行", "步行"];
  ...
  function selectedPoiCacheSig(session) {
    ...
    if (!list.length && session.selected_anchor_poi) {
      list = [session.selected_anchor_poi];
    }
    return list
      .map(function (p) {
  ...
  function hasAnchorSelection(session) {
    var anchor = (session.slots && session.slots.anchor) || "玩";
    if (anchor === "玩") {
      return (session.selected_anchor_pois || []).length > 0;
    }
    return !!(session.selected_anchor_poi || (session.selected_anchor_pois || []).length);
  }
  ...
  function isPlayMultiSelect(session) {
    return ((session.slots && session.slots.anchor) || "玩") === "玩";
  }
  ...
  function startProgressSimulation(etaSeconds) {
    ...
    setProgressStep("frame", "is-done");
    setProgressStep("route", "is-active");
    setProgress(12, "路书框架已就绪，正在编排动线…", etaSeconds);

    if (progressTimer) clearInterval(progressTimer);
    progressTimer = setInterval(function () {
      var elapsed = Date.now() - start;
      var ratio = Math.min(0.72, elapsed / duration);
      var pct = 12 + ratio * 100 * 0.55;
      var remain = Math.max(0, (duration - elapsed) / 1000);
      setProgress(pct, "正在匹配 POI 与交通能耗…", remain);
    }, 120);
  }

  function finishProgress(generationMs) {
    if (progressTimer) {
    ...
    setProgress(82, "正在写入 AI 行程诊断…", 1);
    setTimeout(function () {
      setProgressStep("diag", "is-done");
      setProgressStep("render", "is-active");
      setProgress(96, "正在渲染页面…", 0.5);
      setTimeout(function () {
        setProgressStep("render", "is-done");
  ...
  function renderTransportLegPreview(legs) {
    return (legs || [])
      .map(function (leg) {
        var fromType = POI_TYPE_LABEL[leg.from_type] || leg.from_type || "";
        var toType = POI_TYPE_LABEL[leg.to_type] || leg.to_type || "";
        ...
        return (
          ...
          escapeHtml(toType) +
          "）</p>" +
          '<p class="transport-leg__mode">推荐 ' +
          escapeHtml(leg.selected_mode || leg.recommended_mode) +
          " · " +
          (leg.tran_minutes || 0) +
          "min · 消耗 " +
          (leg.tran_cost || 0) +
  ...
  function renderTransportEditRows(legs) {
    return (legs || [])
      .map(function (leg) {
        var options = TRANSPORT_MODES.map(function (mode) {
          var selected = (leg.selected_mode || leg.recommended_mode) === mode ? " selected" : "";
          return (
            '<option value="' +
            escapeHtml(mode) +
            '"' +
        ...
        return (
          '<div class="transport-edit-row" data-leg-id="' +
          escapeHtml(leg.leg_id) +
          '">' +
          '<label class="transport-edit-row__label">Day ' +
          leg.day_index +
          " · " +
          escapeHtml(leg.from_name) +
          " → " +
  ...
  function collectTransportModesFromEdit() {
    var modes = {};
    if (!transportEditList) return modes;
    transportEditList.querySelectorAll(".transport-mode-select").forEach(function (sel) {
      modes[sel.dataset.legId] = sel.value;
    });
    return modes;
  }

  function renderAnchorPickCard(pick, multiMode, selected) {
    var sketch = pickSketchUrl(pick);
    var selectedCls = selected ? " anchor-pick--selected" : "";
    var check = multiMode
      ? '<span class="anchor-pick__check" aria-hidden="true">' + (selected ? "✓" : "") + "</span>"
      : "";
    return (
      '<button type="button" class="anchor-pick' +
      selectedCls +
      '" data-rank="' +
      pick.rank +
      '" data-poi-id="' +
      ...
      '">' +
      check +
      '<div class="anchor-pick__thumb">' +
      '<img class="anchor-pick__sketch" src="' +
      escapeHtml(sketch) +
      '" alt="" loading="lazy" />' +
      "</div>" +
      '<div class="anchor-pick__body">' +
      '<span class="anchor-pick__rank">推荐 ' +
      pick.rank +
      "</span>" +
      '<p class="anchor-pick__name">' +
      escapeHtml(pick.name) +
      "</p>" +
      '<p class="anchor-pick__meta">评分 ' +
      (pick.rating != null ? pick.rating.toFixed(1) : "—") +
      " · " +
      escapeHtml(pick.visit_time || "") +
      "</p>" +
      '<p class="anchor-pick__reason">' +
      escapeHtml(pick.reason || "") +
      "</p></div></button>"
    );
  }

  function ensureAnchorPoiSelected(session) {
    return new Promise(function (resolve) {
      if (hasAnchorSelection(session)) {
        resolve(session);
        return;
      }
      var picks = session.anchor_recommendations || [];
      if (!picks.length || !anchorPickerModal || !anchorPicksRoot) {
        resolve(session);
        return;
      }

      var multiMode = isPlayMultiSelect(session);
      var anchor = (session.slots && session.slots.anchor) || "玩";
      var typeLabel = POI_TYPE_LABEL[anchor] || "POI";
      if (anchorPickerSub) {
        anchorPickerSub.textContent = multiMode
          ? "已锁定第一锚点「玩 · 景点」，可从推荐中多选，所选景点都会出现在路书里（至少选 1 个）。"
          : "已锁定第一锚点「" +
            anchor +
            " · " +
            typeLabel +
      ...
      function syncMultiFooter() {
        var count = Object.keys(selectedRanks).length;
        if (anchorPickerCount) {
          anchorPickerCount.textContent = "已选 " + count + " 个景点";
        }
        if (anchorPickerConfirm) {
          anchorPickerConfirm.disabled = count < 1;
        }
      }

      function renderCards() {
        anchorPicksRoot.innerHTML = picks
          .map(function (pick) {
            return renderAnchorPickCard(pick, multiMode, !!selectedRanks[pick.rank]);
          })
          .join("");
      }

      renderCards();
      if (anchorPickerFooter) {
        anchorPickerFooter.hidden = !multiMode;
      }
      anchorPickerModal.hidden = false;
      anchorPickerModal.removeAttribute("hidden");

      if (multiMode) {
        syncMultiFooter();
        anchorPicksRoot.onclick = function (e) {
          var btn = e.target.closest(".anchor-pick");
          if (!btn) return;
          var rank = Number(btn.dataset.rank);
          if (selectedRanks[rank]) {
            delete selectedRanks[rank];
          } else {
        ...
        if (anchorPickerConfirm) {
          anchorPickerConfirm.onclick = function () {
            ...
            saveSession(session);
            anchorPickerModal.hidden = true;
            anchorPickerModal.setAttribute("hidden", "");
            anchorPicksRoot.onclick = null;
            if (anchorPickerConfirm) anchorPickerConfirm.onclick = null;
            resolve(session);
          };
        }
        return;
      ...
      anchorPicksRoot.onclick = function (e) {
        ...
        if (!btn) return;
        var rank = Number(btn.dataset.rank);
        var chosen = picks.find(function (p) {
          return p.rank === rank;
        });
        if (!chosen) return;
        session.selected_anchor_poi = chosen;
        session.selected_anchor_pois = [chosen];
        saveSession(session);
        anchorPickerModal.hidden = true;
        anchorPickerModal.setAttribute("hidden", "");
  ...
  function ensureTransportConfirmed(session, currentPlan) {
    return new Promise(function (resolve, reject) {
      if (session.transport_confirmed) {
        resolve({ session: session, plan: currentPlan });
        return;
      }

      var legs = (currentPlan && currentPlan.transport_legs) || [];
      if (!legs.length || !transportModal) {
      ...
      var anchorNames =
        (session.selected_anchor_pois || [])
          .map(function (p) {
            return p.name;
          })
          .filter(Boolean)
          .join("、") ||
        (session.selected_anchor_poi && session.selected_anchor_poi.name) ||
        (currentPlan.selected_anchor_poi && currentPlan.selected_anchor_poi.name) ||
        "锚点 POI";
      if (transportModalSub) {
        transportModalSub.textContent =
          "已根据「" +
          anchorNames +
          "」与画像偏好生成全程出行方案。景点与酒店切换段已优先推荐，你可接受或逐项修改。";
      }
      if (transportLegsPreview) {
        transportLegsPreview.innerHTML = renderTransportLegPreview(legs);
      }
      if (transportEditList) {
        transportEditList.innerHTML = renderTransportEditRows(legs);
      }
      if (transportEdit) transportEdit.hidden = true;
      if (transportActions) transportActions.hidden = false;
      transportModal.hidden = false;
      transportModal.removeAttribute("hidden");

      function finish(modes, refetch) {
        session.transport_modes = modes || currentPlan.transport_modes || {};
        session.transport_confirmed = true;
        saveSession(session);
        transportModal.hidden = true;
        transportModal.setAttribute("hidden", "");
        if (refetch) {
          fetchPlan(session, 12000)
            .then(function (data) {
              resolve({ session: session, plan: data.plan, generationMs: data.generation_ms });
            })
            .catch(reject);
        } else {
          resolve({ session: session, plan: currentPlan });
      ...
      if (transportAcceptBtn) {
        transportAcceptBtn.onclick = function () {
          finish(currentPlan.transport_modes || {}, false);
        };
      }
      if (transportModifyBtn) {
        transportModifyBtn.onclick = function () {
          if (transportEdit) transportEdit.hidden = false;
          if (transportActions) transportActions.hidden = true;
        };
      }
      if (transportEditConfirmBtn) {
        transportEditConfirmBtn.onclick = function () {
          finish(collectTransportModesFromEdit(), true);
        };
      }
    });
  }

  function bootPlanPipeline(session, shellAlreadyBooted) {
    ...
    fetchPlan(session, 12000)
      .then(function (data) {
        return ensureTransportConfirmed(session, data.plan).then(function (result) {
          return {
            plan: result.plan,
            generationMs: result.generationMs != null ? result.generationMs : data.generation_ms,
            session: result.session,
            cacheKey: planCacheKey(result.session),
          };
        });
      })
      .then(function (payload) {
        try {
          sessionStorage.setItem(STORAGE_PLAN_KEY, payload.cacheKey);
        } catch (e) {}
        hydrateFullPlan(payload.plan, payload.generationMs);
      })
      .catch(function (err) {
  ...
  function hydrateFullPlan(fullPlan, generationMs) {
    plan = fullPlan;
    try {
  ...
  function init() {
    try {
      ...
      ensureAnchorPoiSelected(session)
        .then(function (sess) {
          bootPlanPipeline(sess, true);
        })
        .catch(function (err) {
          showError(err.message || "路书生成失败");
        });
